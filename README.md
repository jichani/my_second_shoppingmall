...
클론 코딩한 기존 페이지는 반응형이 아니라서, 모바일, PC 개개의 페이지가 존재한다.<br>
이를 하나의 반응형 페이지로 만들어 사용하면 유지 보수하기가 편하다는 장점이 있다.

1. 부트스트랩의 다양한 컴포넌트와 클래스 사용 및 부트스트랩 Grid 이해.
2. 미디어쿼리를 이용한 반응형 웹페이지 구축.
3. 자바스크립트 이용한 여러 기능 구현.
